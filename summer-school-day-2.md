---
title: "Summer School — Day 2"
permalink: /summer-school/day-2/
map_enabled: true
---

<section>
  <h1>Summer School — Day 2</h1>
  <p class="muted">
    Venue: <strong>{{ site.data.event.summer_school.venue_name }}</strong>, {{ site.data.event.summer_school.venue_room }}, {{ site.data.event.summer_school.venue_address }}.
  </p>

  {% include summer-school-tabs.html active="day2" %}

  {% assign day = site.data.schedule.days | where: "date", "2026-06-23" | first %}
  <div class="schedule-day" style="margin-top: 12px;">
    <div class="schedule-head">
      <div><strong>{{ day.label }}</strong></div>
      <div class="track">{{ day.track }}</div>
    </div>
    <table class="schedule-table" aria-label="Schedule for {{ day.label }}">
      <tbody>
        {% for item in day.items %}
          <tr>
            <td class="time">{{ item.time }}</td>
            <td>
              {% if item.speaker %}
                <strong>{{ item.speaker }}</strong><br>
                <em>Title: {{ item.talk_title }}</em>
                {% if item.lecture_notes_url %}
                  <br><a class="lecture-notes-link" href="{{ item.lecture_notes_url | relative_url }}">{{ item.lecture_notes_label | default: "Lecture notes" }}</a>
                {% endif %}
              {% else %}
                {{ item.title }}
              {% endif %}
            </td>
          </tr>
        {% endfor %}
      </tbody>
    </table>
  </div>
</section>

{% include summer-venue-map.html %}
